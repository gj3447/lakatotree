...
# Inspect assignments inside one SET clause only.  The former ``SET.*verdict``
# scanner crossed later WHERE clauses and mistook a guarded node_state repair
# for a verdict transition merely because its predicate mentioned a verdict.
_SET_CLAUSE = re.compile(
    r"\bSET\b(?P<body>[\s\S]*?)"
    r"(?=\b(?:OPTIONAL\s+MATCH|MATCH|WHERE|WITH|RETURN|MERGE|REMOVE|"
    r"FOREACH|UNWIND|CALL|CREATE|DETACH|DELETE|ORDER\s+BY|LIMIT)\b|$)",
    re.IGNORECASE,
)
    ...
    for rel in _SCAN_DIRS:
        for py in (_ROOT / rel).rglob("*.py"):
            ...
            for cy in _cypher_strings(ast.parse(py.read_text(), filename=relpath)):
                for clause in _SET_CLAUSE.finditer(cy):
                    if not _TRANSITION_ASSIGNMENT.search(clause.group("body")):
                        continue
                    n_transitions += 1
                    # Only text before this exact transition SET may satisfy
                    # the guard; its own assignment literal cannot self-pass.
                    pre_set = cy[:clause.start()]
                    if not any(tok in pre_set for tok in _GUARD_TOKENS):
                        snippet = " ".join(cy.split())[:120]
                        violations.append(
                            f"{relpath}: 무가드 verdict-전이 SET → …{snippet}…"
                        )
