...
게이트: oo_sink.enabled() (AIRO_LOGS_E2E=1 ∧ OO_PASS) — OFF 면 완전 no-op(로컬 = 조용).
...
#   (/data/kjra/PROJECT/3D/*_ICP_SPEC/evidence/...)의 측정 데이터를 import 시점/런타임에 읽는다. 그 데이터가
...
_EXT_3D_EVIDENCE_ROOT = "/data/kjra/PROJECT/3D"
collect_ignore = []
if not os.path.isdir(_EXT_3D_EVIDENCE_ROOT):
    collect_ignore += [
        "test_sx3i_engine_judged.py",        # import 시점 EV read (BLOOM_NODES=bloom_nodes())
        "test_three_d_detection.py",         # → three_d_detection → sx3i_engine_judged
        "test_three_d_dashboard.py",
        "test_three_d_engine_canonical.py",
        "test_three_d_tree_invariants.py",
        "test_record_judge.py",              # 런타임 audit_dir("/data/kjra/PROJECT/3D/...") 무가드
        "test_lx3_engine_judged.py",         # 런타임 judged_nodes() → LX3 evidence read
    ]
