...
def test_20260630_branch_and_measurement_gate_are_longinus_bound() -> None:
    ...
    for item in decisions.values():
        ...
        assert item["parent"] == "prismv2_port365"
        if evidence_root.exists():
            assert Path(item["sourcePath"].split(":", 1)[0]).exists(), item

    ids = {item["sourceId"] for item in data["prismv2_bindings"]}
    assert "PrismV2.DevelopCanonicalBranch.20260630" in ids
    assert "prismv2.dc375.measure_lot.view_health" in ids
    assert "prismv2.dc375.TabBoltStepPolicy.zspread_suspect_mm" in ids
