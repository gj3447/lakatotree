"""consumer_b/consumer_a Longinus knowledge pack drift checks.

The manifest is intentionally a development artifact: it lets Lakatotree keep
the consumer_b segmentation contracts, consumer_a source references, and judge entrypoints
...
MANIFEST = ROOT / "docs" / "bpc_consumer_a_longinus_manifest.json"
...
def test_contract_targets_cover_consumer_a_integration_surface() -> None:
    data = _load()
...
def test_workspace_evidence_is_declared_but_not_required_for_public_ci() -> None:
    ...
    if not (workspace / "consumer_b").exists():
        return
...
def test_manifest_contract_judge_reports_zero_missing_contracts() -> None:
    script = ROOT / "judges" / "bpc_consumer_a_manifest_contracts.py"
