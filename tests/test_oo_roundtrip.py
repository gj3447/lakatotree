...
이 파일이 그 구멍을 닫는다 — ooptdd 0.3.0 backend conformance kit(실제 ship→query 왕복)으로:
  ...
  · gated(real store): 외부 백엔드 1개로 positive 외부 왕복(AIRO_LOGS_E2E=1 + OOPTDD_E2E_BACKEND).
...
@pytest.mark.skipif(os.getenv('AIRO_LOGS_E2E') != '1' or not os.getenv('OOPTDD_E2E_BACKEND'),
                    reason='oo e2e off — 실외부 store positive 왕복 (AIRO_LOGS_E2E=1 + OOPTDD_E2E_BACKEND 설정)')
