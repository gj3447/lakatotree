...
def test_service_create_tree_merges_tree_and_returns_ok():
    captured: list = []
    out = _svc(captured).create_tree("T", CreateTreeIn(
        title="t",
        hard_core="HC",
        frontier_rule="FR",
    ...
    assert any(p.get("coverage_status") == "exhaustive" for _, p in flat)


def test_create_tree_coverage_contract_rejects_unearned_exhaustive_claims():
    with pytest.raises(ValidationError, match="coverage_statement"):
        CreateTreeIn(coverage_status="exhaustive")

    with pytest.raises(ValidationError, match="coverage_backlog"):
        CreateTreeIn(
            coverage_status="exhaustive",
    ...
    with pytest.raises(ValidationError):
        CreateTreeIn(coverage_status="complete")


def test_create_tree_legacy_backlog_is_canonicalized_to_partial():
    spec = CreateTreeIn(coverage_backlog=["src/unread.py"])
    assert spec.coverage_status == "partial"
...
def test_mcp_create_tree_tool_posts_to_tree_route(monkeypatch):
    ...
    mcp.create_tree("T", hard_core="HC", frontier_rule="FR", coverage_status="partial",
                    coverage_backlog_csv="a, b")
    ...
    assert seen[0][1]["coverage_status"] == "partial"
...
def test_cli_tree_create_posts(monkeypatch):
    ...
    cli.main(["tree-create", "T", "--hard-core", "HC", "--frontier-rule", "FR",
              "--coverage-status", "exhaustive",
              "--coverage-statement", "scope=src/** @ commit abc123"])
    assert calls[0][0] == "POST" and calls[0][1] == "/api/tree/T"
    assert calls[0][2]["hard_core"] == "HC" and calls[0][2]["frontier_rule"] == "FR"
    assert calls[0][2]["coverage_status"] == "exhaustive"
