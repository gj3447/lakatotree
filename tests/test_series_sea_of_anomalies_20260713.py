...
수정: 'progressive' 게이트를 zero-anomaly 에서 *진보 우세*(progressive_count > 모든 counter-signal 합)로 —
매직넘버 신규 도입 없이 count 비교로 "진보가 troubles 를 능가하면 진보"(sea-of-anomalies)를 이행.
degenerating 게이트는 불변(이상 누적·nonprogressive 우세 시). 이 관용 이전엔 anomaly>0 이 즉시 progressive 를
막았으므로, 게이트를 zero-anomaly 로 되돌리면 첫 가드 RED.
