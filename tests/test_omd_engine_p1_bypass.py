...
PROBLEM (omd/docs/FEEDBACK_problems_20260630.md §P1)
  OMD 의 "분열=0 사전보장"은 advisory — 에이전트가 opt-in 해야만 성립한다. 실측: BPC 세션들이
...
ORACLES
  guard_defect (test_trailer_blind_audit_admits_bypass_classify_fails_loud):
      ...
      직접커밋·수동머지·위조 trailer 전부에 GO 를 준다(BPC 사고의 재현). 원리 분류기는
...
pytestmark = pytest.mark.skipif(
    not os.path.isdir("/data/kjra/PROJECT/PI/omd"),
...
_HISTORY = (
    ...
    _Commit("c2", 1, False),                      # 직접커밋 우회 (BPC 사고 유형)
...
_OMD_ROOT = "/data/kjra/PROJECT/PI/omd"
