...
def test_current_verdict_is_fold_over_receipt_chain():
    ...
    assert out['verdict'] == 'progressive_unverified', out   # finding A: dogfood default (no Lakatos qual)
    ...
    assert kg.receipts[0]['verdict'] == 'progressive_unverified'   # finding A: sealed receipt carries the emitted verdict

    # (2) rebuild_verify: 체인 fold 재유도 == e.verdict 캐시(재유도가 판관, 캐시 신뢰 아님).
    v = svc.verify_verdict_chain('T', 'seam')
    assert v['ok'] and v['from_receipt'] and v['rederived'] == 'progressive_unverified', v   # finding A
    ...
    assert v2['ok'] is False and v2['rederived'] == 'progressive_unverified', v2   # finding A; 체인이 진실을 보존, 변조 검출
