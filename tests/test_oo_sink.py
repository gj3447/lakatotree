"""oo 전송 sink TDD(LTDD) — env-gate(AIRO_LOGS_E2E∧OO_PASS) + opener 주입 + 트레이스 빌더.
...
def _enable(monkeypatch, pw='secret'):
    monkeypatch.setenv('AIRO_LOGS_E2E', '1')
...
def test_ship_posts_json_to_oo_stream_with_basic_auth(monkeypatch):
    ...
    monkeypatch.setenv('OO_USER', 'root@airo.local')
    ...
    assert raw == 'root@airo.local:hunter2'
