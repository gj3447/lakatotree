"""git-흡수 M0 landed guards — GitNexus 구조 교차검증 (흡수설계 전수성의 방법론 근거).
  ...
  guard_mechanism(novel축) : test_chokepoint_anchors_present_in_shared_kg
        — 초크포인트 6개가 공유 airo KG 에 chokepoint:true + ABSORPTION_ANCHOR→(실재 G-노드) 로
          살아 있다(눈-검증 아닌 *구조* 증거가 흡수 프로그램 노드에 물리적으로 연결).

증거원: 공유 airo KG(importBatch git-source-structure-20260702 + -m0complete) — GitNexus 인덱스
...
_KG_URL = os.environ.get('AIRO_KG_MCP_URL', 'http://10.147.17.7:55013/mcp/')
_GIT_SRC = Path(os.environ.get('LAKATOS_GIT_SRC', '/data/kjra/PROJECT/PI/GIT/git'))
...
def _kg_or_skip(query: str) -> list[dict]:
    rows = _kg_read(query)
    if rows is None:
        pytest.skip(f'공유 airo KG 미접근({_KG_URL}) — M0 증거는 KG 에 산다(오프라인/클린클론 skip)')
    return rows
