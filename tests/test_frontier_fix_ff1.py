...
def test_cross_metric_novel_requires_server_readback_or_sha(tmp_path):
    ...
    p = _params(cap)
    assert p['v'] == 'progressive_unverified', p   # finding A: dogfood default (no Lakatos qual)
...
def test_policy_is_opt_in_off_by_default_non_breaking():
    ...
    assert _params(cap)['v'] == 'progressive_unverified'   # finding A: dogfood default (no Lakatos qual)
