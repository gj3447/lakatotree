...
정직하게 고정한다:
  ...
  (3) CORPUS(정직한 한계) — 현 dogfood 프로그램(euler/halcon3d 의 flip-호환 run())에서 *실제 flip 은
...
FLIP_COMPATIBLE_DOGFOOD = ('euler_polyhedron_programme', 'halcon3d_inspection_programme')
...
def test_layer_flip_audit_over_dogfood_is_zero_honest_corpus_finding():
    """CORPUS: flip-호환 dogfood(euler/halcon3d)에서 stack 3층 flip 합계 = 0(정직한 발견을 freeze).
