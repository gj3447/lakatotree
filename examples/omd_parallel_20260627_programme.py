...
_ROOT = "/data/kjra/PROJECT/PI/lakatotree"
...
NODES_DEF: tuple[ParallelNode, ...] = (
    ...
    ParallelNode(
        ...
        prediction=Prediction(metric_name="single_phase_merge_crash_strands_or_double_applies", direction="lower",
                              baseline_value=1.0, noise_band=0.0,
                              novel_prediction="merge_sha-before-release + token 배타가 TLC 로 모델체크돼 crash 시 정확히-한번 복구",
                              closes_question="q-omd-2pc"),
        novel_target=NovelTarget(metric_name="omd_connect_2pc_invariants_model_checked", direction="higher", threshold=1.0),
    ...
    ParallelNode(
        ...
        prediction=Prediction(metric_name="pure_ttl_lease_admits_stale_writer", direction="lower",
                              baseline_value=1.0, noise_band=0.0,
                              novel_prediction="fence 단조성/무stale-mutate 가 TLC 로 모델체크돼 stale writer 가 봉쇄된다",
                              closes_question="q-omd-fencing"),
        novel_target=NovelTarget(metric_name="omd_fencing_invariants_model_checked", direction="higher", threshold=1.0),
    ...
    ParallelNode(
        ...
        prediction=Prediction(metric_name="dual_coordinator_split_brain_double_write", direction="lower",
                              baseline_value=1.0, noise_band=0.0,
                              novel_prediction="단일리더/무stale-mutate/stale-못됨-리더 가 TLC 로 모델체크돼 split-brain 봉쇄",
                              closes_question="q-omd-leader"),
        novel_target=NovelTarget(metric_name="omd_leader_tla_single_writer_invariants_checked", direction="higher", threshold=1.0),
