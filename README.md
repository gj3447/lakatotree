...
`engine` `verdicts` `node_state` `grounding` `trust` `claim` `world_gates` `harness` `harness_run` `longinus` `semantic_surface` `cli` `mcp_server` `eureka` `facts` `research_import` `provenance_backfill` `ontology` `assurance`
...
(`AIRO_LOGS_E2E`/`OO_PASS` unset) it is a complete no-op (local = quiet); ship failures never break
the build (observation does not change the verdict).
```bash
AIRO_LOGS_E2E=1 OO_PASS=*** python -m pytest tests/ -q   # ship traces (gate on)
```
