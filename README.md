...
- `verdicts` verdict-vocabulary single source of truth · `node_state` explicit KG node lifecycle FSM · `grounding` all constants with tier honesty (literature / policy-in-scale / policy) · `coverage` fail-closed `unknown|partial|exhaustive` declaration shared by write/read paths
...
- `metrics.coverage.status` exposes `unknown|partial|exhaustive` alongside the backlog;
  only an explicit scoped declaration with an empty backlog earns `exhaustive`.
