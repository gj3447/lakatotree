...
lakatos/            순수 판결·지표 모듈 (I/O 0, 어디서든 동일 판정) — 이론 기반 = THEORY.md
  ...
  lineage.py        ★데이터 계보 — 버퍼는 임시, 완성본은 root data 서 재생성. stale 감지. LINEAGE.md
...
tests/              판결/엔진 규칙 TDD (83 케이스 — 규칙 변경은 RED 부터)
```
...
## 기동/사용
```bash
python -m pytest tests/ -q      # 엔진 검증 (83 케이스)
...
예: BPC는 root=ZDF인 첫 프로젝트 인스턴스일 뿐이다. KG: `(:LegionCommander)-[:INSTANTIATED_AS]->(:LakatoProject)`.
