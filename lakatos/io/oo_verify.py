...
def _roundtrip(*, cid: str, expect_total: int, writer, reader,
    ...
    반환은 verify_trace 결과 dict. ship/verify 게이트(AIRO_LOGS_E2E∧OO_PASS, OO_URL)는 *in-process
...
def _hermetic_oo_gate():
    ...
    AIRO_LOGS_E2E=1·OO_PASS·OO_URL 만 채우면 enabled()/_endpoint()/verify_trace 가 실 경로를
    타되, opener 주입이 실제 HTTP 를 전부 가로채므로 네트워크는 일어나지 않는다."""
    import os
    keys = {'AIRO_LOGS_E2E': '1', 'OO_PASS': 'roundtrip-dummy',
