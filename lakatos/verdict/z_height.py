"""Longinus BPC Z-height/CAD-surface 게이트 — registration green 이 wrong-z 를 숨기지 못하게 fail-closed.
...
하나라도 결여/뭉갬 → BLOCKED. 그리고 핵심(LGN-BPC-Z-003): registration 이 green(작은 residual)이어도
...
acceptance check(rerunnable): tests/test_z_height_gate.py (실 BPC feature_z_offset_per_hole.json 으로 검증,
부재 시 hermetic skip).
※ frontier(후속): wrong-layer/wrong-axis/free-ICP/panel-only/nearest-triangle/shuffled-ID 음성대조 실행 강제
   배선(별도 바인딩 BPC.ZLayerNegativeControlsExecuted = CANDIDATE).
...
# Required BPC Z Guards 스키마(doc 라인 179-198)의 load-bearing 필드. 하나라도 결여 → 뭉갬 = BLOCKED.
...
def judge_z_height(result) -> ZHeightVerdict:
    """BPC Z 결과 판정. 세 surface 뭉개짐 → BLOCKED; registration green 이 z 오차 숨기면 Z-NOT-CERTIFIED."""
    ...
    if zerr > reg + u_k2:
        return ZHeightVerdict(
            ...
            f"reg+U_k2({reg + u_k2:.3f}mm) 초과 — 정합 metric 이 z-차원을 보증 못함(LGN-BPC-Z-003).")
    if zerr <= u_k2:
        return ZHeightVerdict(
