...
먹이지 않고 verdict 를 손입력(sx3i_icp_programme)하거나 Prediction 을 .py 에 하드코딩(halcon3d)했다.
...
from lakatos.programme import evidence as E          # 정본: examples/_evidence → lakatos.programme.evidence
from lakatos.verdict.judge import Prediction, judge  # 엔진 판결 커널 (in-process)
...
if __name__ == "__main__":
    import json
    d = sys.argv[1] if len(sys.argv) > 1 else "/data/kjra/PROJECT/3D/SX3i_ICP_SPEC/evidence"
    rows = audit_dir(d)
    by = {}
    for r in rows:
