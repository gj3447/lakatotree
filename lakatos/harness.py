...
class LakatoHarness:
    ...
    def run_cycle(self, s: CycleSpec) -> dict:
        prov = {'tree': s.tree, 'tag': s.tag, 'realms': {}}

        # ── 상계(read-only): 인터넷서 불(지식) 훔치기 + 신뢰가중 ──
        evidence, trust = [], 1.0
        ...
        # ── 하계(write): agent 가 노드 생성 + 구조적 예측 사전등록 + 채점 sha ──
        sha = None
        if s.judge_script:
            try:
        ...
        # ── 하계(execute, ground truth): 빌드/TDD — 실패면 게이트로 중단 ──
        if s.build_cmd:
            out, code = self._bash(s.build_cmd)
            prov['build'] = {'cmd': s.build_cmd, 'exit': code, 'tail': out[-120:]}
            if code != 0:
                raise BuildFailed(f'빌드/TDD 실패(exit {code}) — 채점 중단. {out[-120:]}')

        # ── 하계(execute, measure): 채점 스크립트 → metric ──
        metric = None
        if s.judge_cmd:
            out, _ = self._bash(s.judge_cmd)
            metric = _parse_metric(out)
        prov['metric'] = metric

        # ── 하계(write): test_result 제출 → 판결(judge, 인터넷 신뢰 결합) ──
        res = self._http('POST', f'/api/tree/{s.tree}/node/{s.tag}/test_result', {
        ...
        # ── 인간+agent(critique): 의문/반박 등재 → 정당성(Dung) ──
        ...
        prov['standing'] = self._http('GET', f'/api/tree/{s.tree}/node/{s.tag}/standing')

        # ── 이력관리: git sha (코드 버전 관통) ──
        prov['git_sha'] = self._git()
