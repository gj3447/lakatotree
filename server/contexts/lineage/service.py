...
class LineageService:
    ...
    def rebuild_verify(self, artifact: str) -> dict:
        ...
        return dict(
            ...
            note="rebuildable_static=정적 분석상 raw root+현재환경서 재생성 가능(레시피 완전·미stale·env 일치) — 재실행은 안 함(executor rebuild-run 이 실제 영수증). progressive_conditional=env/데이터 바뀜 → 재실행 필요(BPC ZDF Rule #5)",
        )
