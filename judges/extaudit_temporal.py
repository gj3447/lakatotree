...
측정: tests/test_extaudit_temporal.py 전체를 pytest 로 실행해 failed 수를 센다.
...
# KG 거울: LakatosTree_LakatoTree_SelfDev_20260612 / v26_extaudit_temporal
"""
import re
import subprocess
import sys
...
def count_failures() -> int:
    p = subprocess.run(
        [sys.executable, '-m', 'pytest', 'tests/test_extaudit_temporal.py', '-q', '--tb=no'],
