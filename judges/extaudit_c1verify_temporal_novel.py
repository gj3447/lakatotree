#!/usr/bin/env python3
"""EXTAUDIT S8 novel 채점기 — 전체 회귀 실패 수 (독립 결과: MeasurementLock 도입이 기존 계약 무파손).

주 메트릭(신규 가드)과 독립 — 전체 스위트 failed+error. novel_threshold<1(즉 0): 사이드카 설계
(receipt v3 불변)라면 기존 골든 전부 생존해야 한다는 사전 예측.
