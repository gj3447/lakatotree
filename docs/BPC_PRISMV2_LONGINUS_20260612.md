# BPC PrismV2 Segmentation Longinus Pack
...
> sourcePath=`docs/BPC_PRISMV2_LONGINUS_20260612.md:1`

이 문서는 BPC segmentation/prismv2 기반지식을 Lakatotree 개발 루프에 꽂는
Longinus 기준점이다. 목표는 Lakatotree가 BPC 작업을 단순 메모가 아니라
...
`BPC` id 0 and `ZIG` id 7 are training/context classes, not production measurement classes.
...
- tag: `seg_v3_prismv2_contract`
- parent: current BPC canonical node
- metric: `contract_missing_count`
- direction: `lower`
- novel target: class-map drift count equals `0` and required contracts exist
- build command: `python -m pytest tests/test_bpc_prismv2_longinus_manifest.py -q`

After prismv2 implementation starts, replace the metric with result-level judges:
...
- 실 작업사: jg_bpc `kjra` LTDD 캠페인 W1~W5 (2026-06-14), Windows tester PC(DESKTOP-SNAP5I3) 133 pytest green.

**Hard core 추가(운반·안전):**

- bulk numpy(30MB+)는 proto bytes 금지 — `ShmHandle`(`/dev/shm/prism/<uuid>.npz` + shape/dtype/sha256).
...
**analysis-contract 출력 ↔ jg_bpc 산출(progressive 노드):**

| # | 출력 | 노드 | jg_bpc surface |
|---|---|---|---|
...
**OPEN frontier = 실 잔여(가짜 green 아님):** `q_flight_consumer`(per-view Flight 실시간 라우팅, prismv2 F4),
`q_w2prod`(DC375 authoritative measure_lot per-view snr 운반), `q_seg`, `q_label_weight`.
...
- `BPC/2dmaskedbase/ultracode1/GEOMETRY_TRUTH.json:5`: `CUP` truth.
- `BPC/2dmaskedbase/ultracode1/GEOMETRY_TRUTH.json:76`: `PLATE_HOLE` truth.
- `BPC/2dmaskedbase/ultracode1/GEOMETRY_TRUTH.json:251`: `OUTER_HOLE` truth.
- `BPC/2dmaskedbase/ultracode1/GEOMETRY_TRUTH.json:331`: `EXTERNAL_HOLE` truth.
- `BPC/2dmaskedbase/ultracode1/GEOMETRY_TRUTH.json:802`: `TAB_BOLT`/washer truth.
- `BPC/2dmaskedbase/ultracode1/CAD_FACTOR_SPEC_v1.md:5`: raw residuals against CAD primitives.
- `BPC/2dmaskedbase/ultracode1/BASE_PLANE_WASHER_Z.md:93`: washer z production fields.
- `BPC/seg_train/dataset_v3/bpc_seg_v3.yaml:6`: YOLO class map.
- `BPC/scripts/271_zlayer_clean_seg.py:70`: z-layer class generation map.
- `BPC/seg_train/train_v3.log:817`: segmentation validation metrics.
- `BPC/2dmaskedbase/ultracode1/SOLVER_PKG_v12/README.md:19`: v12 solver decisions.

## Development Checklist

1. Add a prismv2 neural registry entry for the BPC YOLO segmentation model.
...
The machine-readable form of this pack is `docs/bpc_prismv2_longinus_manifest.json`.

## 2026-06-30 Longinus Binding

The active BPC PrismV2 branch/capability update is bound under:
...
- parent: `prismv2_port365`

The binding pins two operational facts:

1. PrismV2 multi-agent development baseline is `origin/develop`; stale `kjra`
   or feature worktrees must merge/rebase `origin/develop` before integration.
2. BPC product verdicts come from feature-local geometry plus measurement
