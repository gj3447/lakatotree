...
> - 이 박스 zt IP = `10.147.17.136/24` (iface `zt5zmfe7ey`), airo zt망 가입됨.
> - Neo4j+MCP 호스트 = **`10.147.17.7`** (= 내부망 "macmini").
> - zt에 노출된 건 **MCP HTTP `10.147.17.7:55013`뿐** — Neo4j bolt(7687)·Mongo(27017)는 **미노출**
>   (zt /24 전수스캔 0건). 그래서 `bolt://` 직접연결이 필요한 서버는 **다른 zt 호스트(이 박스 포함)에서
>   못 붙는다.** airo-neo4j MCP(`type:http`)만이 그 Neo4j에 닿는 유일한 zt 경로다.
> - `10.147.17.7:22` ssh OPEN이나 이 박스엔 키/비번 없음 → 원격 기동 불가.
>
> **결론: 서버는 `10.147.17.7` 본인 세션에서 띄운다.** 거기선 Neo4j가 localhost:7687.
...
> #   LAKATOTREE_URL=http://10.147.17.7:55170 python -m lakatos.cli metrics LakatosTree_VerdictProvenanceGate_20260620
> ```
...
> 전제**다. airo-neo4j MCP(Cypher)만으로도 대부분 조회/계산은 되지만, 풀 엔진은 서버 경유.

## 왜 이 박스(`/data/kjra` = airo/prismv2)에선 못 띄우나
...
(`ping 100.64.0.3` 100% loss). airo-neo4j **MCP** 는 동작하지만 그건 *MCP 호스트*가 Neo4j 에
...
> ⚠️ `server/run.sh`(run_internal 아님)는 **다른 셋업용**이다 — `/mnt/hdd/kjra/vision3d_test/.env`
> 를 source 하고 `~/.claude/settings.json['env']['NEO4J_*']` + 로컬 docker 컨테이너 `postgresql`
> (포트 55100)를 가정한다. 이 환경엔 그 .env 도, 그 컨테이너 이름(여긴 `airo-postgres`)도 없으니
...
## 서버 없이 지금 가능한 것 (airo-neo4j MCP / Cypher)

서버를 못 띄우는 동안에도 **데이터는 전부 airo Neo4j 에 있으므로** MCP Cypher 로:
